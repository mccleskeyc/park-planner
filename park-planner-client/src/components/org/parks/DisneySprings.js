...
class DisneySprings extends Component {
    render() {
        ...
        const filteredPlans = plans.filter(plan => plan.props.park === "Disney Springs")
        if (filteredPlans.length > 0) {
            return (
                <div className="container">
                <div className="content">
                    <h1>Disney Springs Plans</h1>
                        {filteredPlans}
                        </div>
                </div>
            )} else 
...
export default connect(mapStateToProps)(DisneySprings)
